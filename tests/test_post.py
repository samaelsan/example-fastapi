...
def test_get_one_post(authorized_client, test_posts):
    ...
    assert post.Post.owner.id == test_posts[0].owner_id

@pytest.mark.parametrize("title, content, published", [
    ("awsome new title", "awesome new content", True),
    ("favorite pizza", "I love canadian", True),
    ("Talles skyscrapers", "wahoo", True),
])
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete("/posts/80000000")
    assert res.status_code == 404
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put("/posts/80000000", json=data)
    assert res.status_code == 404